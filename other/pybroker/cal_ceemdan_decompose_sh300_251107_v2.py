...
# =========================
# 4. Long-only timing signal (no short)
# =========================

def build_long_only_timing_signal(prices, trend_part, ma_window=20, slope_lookback=3):
    """
    Long-only timing rule based on CEEMDAN trend:

    - Position = 1 (long) when:
        trend > MA(trend)  and  trend slope > 0
    - Position = 0 (flat) otherwise.
    - NO SHORT positions (no -1).

    Uses previous day's signal on today's return to avoid look-ahead.
...
def print_performance(name, perf):
    ...
    print(f"\n=== {name} Performance ===")
    print(f"Total return:      {fmt_pct(tr)}")
    print(f"Annualized return: {fmt_pct(ar)}")
    print(f"Sharpe ratio:      {fmt_num(sh)}")
...
def main():
    ...
    # 3) Aggregate components (you can tweak hf_imf_count / trend_imf_from)
    hf_part, trend_part = aggregate_components(
        imfs,
        residue,
        hf_imf_count=3,   # IMF1–3 as high-frequency
        trend_imf_from=4  # IMF4+ & residue as trend
    )

    # 4) Long-only timing strategy
    signal, strat_equity = build_long_only_timing_signal(
        values,
        trend_part,
        ma_window=20,
        slope_lookback=3
    ...
    print_performance("CEEMDAN Long-only Timing Strategy", perf_strat)
    ...
    # =========================
    # Plot 3: Timing signal on price (long-only)
    # =========================
    fig3, ax3 = plt.subplots(figsize=(12, 5))
    ax3.plot(t, values, label="Original close", linewidth=1.0)
    long_idx = np.where(signal == 1)[0]
    ax3.scatter(long_idx, values[long_idx],
                marker="^", s=20, label="Long", alpha=0.7)
    ax3.set_title("CSI 300 - CEEMDAN-based long-only timing signal")
    ax3.set_xlabel("Sample index (time order)")
    ax3.set_ylabel("Index level")
    ax3.legend()
    plt.tight_layout()
    fig3.savefig(os.path.join(output_dir, "csi300_signal_long_only.png"), dpi=200)
    plt.show()

    # =========================
    # Plot 4: Strategy equity curve (long-only)
    # =========================
    fig4, ax4 = plt.subplots(figsize=(12, 4))
    ax4.plot(t, strat_equity, linewidth=1.0)
    ax4.set_title("CEEMDAN long-only timing strategy - equity curve")
    ax4.set_xlabel("Sample index (time order)")
    ax4.set_ylabel("Equity (normalized)")
    plt.tight_layout()
    fig4.savefig(os.path.join(output_dir, "csi300_equity_strategy_long_only.png"), dpi=200)
    plt.show()

    # =========================
    # Plot 5: Long-only strategy vs Buy & Hold
    # =========================
    fig5, ax5 = plt.subplots(figsize=(12, 4))
    ax5.plot(t, bh_equity, label="Buy & Hold (CSI 300)", linewidth=1.0)
    ax5.plot(t, strat_equity, label="Long-only timing strategy", linewidth=1.2)
    ax5.set_title("CSI 300 - Long-only timing vs Buy & Hold")
    ax5.set_xlabel("Sample index (time order)")
    ax5.set_ylabel("Equity (normalized)")
    ax5.legend()
    plt.tight_layout()
    fig5.savefig(os.path.join(output_dir, "csi300_equity_long_vs_bh.png"), dpi=200)
    plt.show()
