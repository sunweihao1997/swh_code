...
# 索引必须是 DatetimeIndex，且无重复
print(stock_data.index.dtype, stock_data.index.is_monotonic_increasing, stock_data.index.has_duplicates)

# 必要列名必须小写：high/low/close
print(stock_data.columns)

# swings 要与 stock_data 完全对齐
print(zigzag_result.index.equals(stock_data.index))

print("swing_high count:", zigzag_result['swing_high'].notna().sum())
print("swing_low  count:", zigzag_result['swing_low' ].notna().sum())

print("first non-NaN high at:", zigzag_result['swing_high'].dropna().index.min())
print("first non-NaN  low at:", zigzag_result['swing_low' ].dropna().index.min())
print("last  non-NaN high at:", zigzag_result['swing_high'].dropna().index.max())
print("last  non-NaN  low at:", zigzag_result['swing_low' ].dropna().index.max())
