...
class BacktestParams:
    ...
    def to_label(self) -> str:
        return f"dw{self.dyn_window_months}_dq{self.dyn_quantile:.2f}_pw{self.pe_window_years}_b{self.band:.2f}"
...
class BacktestEngine:
    ...
    def _simulate_month(
        ...
        """
        # Get macro data up to this month
        valid_macro = self.macro_df[
            (self.macro_df['month_end'] <= month_end) & 
        ...
        current_row = valid_macro[valid_macro['month_end'] == month_end]
        if current_row.empty:
            # Use closest available month
            current_row = valid_macro.iloc[[-1]]
            month_end = current_row['month_end'].iloc[0]
        
        current_gap_ma3 = current_row['gap_ma3'].iloc[0]
        prev_gap_ma3 = valid_macro.iloc[-2]['gap_ma3']
        
        # Dynamic threshold
        dyn_thr = compute_dynamic_threshold(
            self.macro_df, month_end, 
...
def generate_param_grid(
    ...
    params_list = []
    for dw, dq, pw, b, ct, et in combinations:
...
def generate_summary_report(results_df: pd.DataFrame, outdir: Path) -> None:
    ...
    for param in ['dyn_window_months', 'dyn_quantile', 'pe_window_years', 'band']:
        print(f"\n{param}:")
