...
df = pd.read_csv("/home/ubuntu/stock_data/stock_price_test/301528.csv",)
...
swings = find_swings_zigzag(
    df, threshold=0.1,         # 例如 6% 阈值；可按波动调整
    mode="percent",
    use_high_low=True,
    include_last=False
)
...
plt.title(f"ZigZag swings (threshold=6%)")
plt.xlabel("Date"); plt.ylabel("Price")
plt.legend()
plt.grid(alpha=0.3)
plt.tight_layout()
plt.savefig("/home/ubuntu/plot/zigzag_swings.png", dpi=300)
