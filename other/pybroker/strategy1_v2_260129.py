...
def fetch_macro_money_supply() -> pd.DataFrame:
    ...
    # -------------------------------------------------------------------------
    # MINIMAL FIX: force selecting YoY columns for M1/M2
    # -------------------------------------------------------------------------
    def _find_money_yoy_col(target: str) -> Optional[str]:
        """
        target: 'm1' or 'm2'
        Require BOTH:
          - contains M1/M2 identifier
          - contains YoY marker (同比/增速/增长/yoy)
        This avoids mistakenly using balance/level columns like '货币(狭义货币M1)'.
        """
        target = target.lower()
        yoy_markers = ['同比', '增速', '增长', 'yoy']  # broad but safe
        ...
        for col in raw.columns:
            ...
            if target == 'm1':
                if ('m1' not in sl) or ('m2' in sl):
                    continue
                # exclude M10/M11/M12 etc.
                if any(x in sl for x in ['m10', 'm11', 'm12']):
                    continue
            elif target == 'm2':
                if 'm2' not in sl:
                    continue
                # exclude accidental matches to M1
                if 'm1' in sl:
                    continue
            else:
                return None
        ...
        # Prefer shorter name (usually more specific/clean)
    ...
    if m1_col is None or m2_col is None:
        ...
        raise ValueError(
            "Could not identify M1/M2 YoY columns (同比/增速/增长/YoY). "
            f"Found M1: {m1_col}, M2: {m2_col}. "
            "Please check AkShare returned columns."
        )

    logger.info(f"Using columns - Date: {date_col}, M1 YoY: {m1_col}, M2 YoY: {m2_col}")
    ...
    if df['date'].isna().all():
        for fmt in ['%Y年%m月', '%Y-%m', '%Y%m', '%Y.%m', '%Y.%m', '%Y.%m']:
            try:
    ...
    if df.empty:
        raise ValueError("No valid data after parsing macro money supply YoY")

    # Convert to month end for consistent alignment
    df['month_end'] = df['date'] + pd.offsets.MonthEnd(0)

    # Compute gap (YoY spread, in pct points)
