...
This version runs in a continuous loop, screens both A-Share and HK markets, and
sends a single summary email at the end of each full cycle.
"""
...
def get_stock_data(stock_code: str, period: str = "daily", start_date: str = "20230101", end_date: str = "20251231") -> Optional[pd.DataFrame]:
    """
...
def plot_vcp_analysis(df: pd.DataFrame, vcp_result: Dict, breakout_signal: Optional[pd.Series], stock_code: str, save_path: str):
    ...
    fig, axlist = mpf.plot(
        plot_df, type='candle', style='yahoo', title=f'VCP Analysis for {stock_code}',
        ylabel='Price', volume=True, ylabel_lower='Volume', figsize=(18, 10),
...
def format_summary_email_html(patterns_list: List[Dict], breakouts_list: List[Dict]) -> str:
    ...
    alert_time = pd.Timestamp.now(tz='Asia/Singapore').strftime('%Y-%m-%d %H:%M:%S %Z')
    ...
    for i, stock_row in stock_list_df.iterrows():
        ...
        try:
            ...
            if pattern_id not in notified_patterns:
                ...
                # MODIFIED: Only add pattern to summary if its pivot is recent (within 15 days)
...
# =============================================================================
# MAIN EXECUTION (REVISED FOR CONSOLIDATED REPORT)
# =============================================================================
...
def main():
    ...
    while True:
        ...
        # --- A-Shares Screening ---
        print("\n" + "="*80)
        print(f"Starting A-Share Market Scan at {pd.Timestamp.now()}...")
        try:
        ...
        print("\n" + "="*80)
        print(f"Starting Hong Kong Market Scan at {pd.Timestamp.now()}...")
        try:
        ...
        if all_new_patterns or all_new_breakouts:
            ...
            subject = f"VCP Screener Summary - {pd.Timestamp.now(tz='Asia/Singapore').strftime('%Y-%m-%d %H:%M')}"
        ...
        # --- Sleep for 6 hours after completing both markets ---
        print(f"Full screening cycle complete. Sleeping for 12 hours until next cycle...")
        time.sleep(12 * 60 * 60)
