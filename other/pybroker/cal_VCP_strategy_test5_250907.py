"""
2025-09-07 最终版（exec_window 限定）
- 修复 DatetimeIndex & 列名统一
- ATR 收缩（周-日联动）→ 生成执行窗口（exec_window_days）
- ZigZag 拐点 → 回撤段 → VCP 校验
- 仅在“执行窗口内”扫描 VCP 命中；并打印过去5年的全历史命中对照
- 可视化：价格 + ZigZag + pairs + 窗口内 VCP 高亮 + pivot 虚线
"""
...
def pairs_in_window(pairs: list, start: pd.Timestamp, end: pd.Timestamp) -> list:
    """仅保留 hi_t 与 lo_t 都落在窗口内的 pairs。"""
    return [p for p in pairs if (p[0] >= start and p[2] <= end)]
...
def find_vcp_in_windows(
    ...
    *,
    ...
    for (ws, we) in windows:
        pw = pairs_in_window(pairs, ws, we)
        if len(pw) < need_n:
            continue
...
if __name__ == "__main__":
    ...
    exec_window_days = 100  # 周线触发后，日线执行窗口
    ...
    recent_chk = is_valid_vcp_sequence_v2(
        ...
        low_mode="atr", low_atr_mult=0.3, atr_series=atr20,
