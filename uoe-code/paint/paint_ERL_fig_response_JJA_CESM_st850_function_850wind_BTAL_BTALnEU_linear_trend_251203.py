...
#    cmap = cmr.prinsenvlag
    ...
    im  =  ax.contourf(cyclic_lon, lat, cyclic_sfd_vint, levels=21, cmap=newcmp, alpha=1, extend='both', norm=norm, transform=ccrs.PlateCarree())
...
#    level2    =  np.array([-5, -4.5, -4, -3.5, -3, -2.5, -2, -1.5, -1, -0.5, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5])
    plot_diff_slp_wind(diff_slp=55*gaussian_filter((psl_diff), sigma=1), diff_u=(u_con - u_neu)*55, diff_v=(v_con - v_neu)*55,  left_title='(a)', right_title='CESM_ALL - CESM_noEU', out_path=out_path, pic_name="ERL_fig_response_CESM_st850_850wind_diff_JJA_linear_trend_1901to1955.pdf", level=level2)
    print('Finished')
