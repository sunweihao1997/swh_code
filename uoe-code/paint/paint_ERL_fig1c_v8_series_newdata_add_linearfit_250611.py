...
colors = ['#333333', '#D75455', '#8E72B5', '#6BAE6C', '#F4A261']
#333333
# add linear fit
...
cru_series  = cal_moving_average(cal_moving_average(cal_moving_average(value_cru,  5), 5), 5)
gpcc_series = cal_moving_average(cal_moving_average(cal_moving_average(value_gpcc,  5), 5), 5)

A = np.vstack([x, np.ones(len(x))]).T  # 构造设计矩阵
m_gpcc, c_gpcc = np.linalg.lstsq(A[10:], y_gpcc[10:55], rcond=None)[0]  # 计算最小二乘法解
...
#
#plt.plot(np.linspace(1911, 2000, 90), cru_series[10:],         color=colors[0],            label='CRU'  , linewidth=3)
#plt.plot(np.linspace(1911, 2000, 90), gpcc_series[10:],        color=colors[1],                label='GPCC' , linewidth=3)
...
plt.fill_between(np.linspace(1901, 2000, 100), cal_moving_average(cal_moving_average(cal_moving_average(0 - value_cesm_allstd,  5), 5), 5), cal_moving_average(cal_moving_average(cal_moving_average(0 + value_cesm_allstd,  5), 5), 5), color='lightgrey', alpha=0.7)

plt.legend(loc='lower right', fontsize=30)
plt.savefig('/home/sun/paint/ERL/ERL_fig1c_v8_ts_new_smooth_test.pdf', dpi=450)


# test part
a = cal_moving_average(cal_moving_average(cal_moving_average(value_cesm_all - value_cesm_allstd,  5), 5), 5)
b = cal_moving_average(cal_moving_average(cal_moving_average(value_cesm_all + value_cesm_allstd,  5), 5), 5)
c = cal_moving_average(cal_moving_average(cal_moving_average(value_cesm_all,  5), 5), 5)
d = cal_moving_average(cal_moving_average(cal_moving_average(value_cesm_allstd,  5), 5), 5)
