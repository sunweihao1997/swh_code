...
def cal_single_JJA():
    lat1 = 20 ; lat2 = 28 ; lon1 = 76 ; lon2 = 87
    # === test ===
    ...
    for i in range(8):
        ...
        slope_gpcc, intercept = np.polyfit(np.linspace(1, 55, 55), a[51:(51+55)], 1)
...
def cal_55year_trend_v2(data, varname):
    ...
    JJA_PRECT = signal.detrend(JJA_PRECT) * 86400000
...
#    b, a = scipy.signal.butter(N, Wn, 'lowpass')
    ...
    for j in range(137):
        ...
        #print(j)
        print(j)
        print(JJA_PRECT.shape)
        slope, intercept = np.polyfit(np.linspace(1, 55, 55), JJA_PRECT[j:(j+55)], 1)
        sample[j]  = slope * 60 # decade
...
#    ax.set_title('Box plot with custom whiskers')
    ...
    plt.savefig('/home/sun/paint/ERL/ERL_figs4_v6_Mdata.pdf')
