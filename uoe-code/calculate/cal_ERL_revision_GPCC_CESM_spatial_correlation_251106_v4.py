...
# -*- coding: utf-8 -*-
"""
Spatial pattern comparison between GPCC and CESM-ALL linear trends
with explicit scaling factors, over a user-defined box.

Outputs:
- Area-weighted Pearson correlation (cos(lat)) with 90% CI
- Area-weighted Spearman (rank) correlation with 90% CI
- Sign-agreement fraction (weighted & unweighted)
- Top/Bottom decile (10%) overlap (weighted & unweighted)

Pipeline:
- Load GPCC & ALL
- Interpolate ALL to GPCC grid
- Subset by 4-corner box (supports dateline)
- (Optional) Snap bounds to nearest GPCC grid points
...
ALL_PATH  = "/home/sun/data/download_data/data/analysis_data/Aerosol_Research_CESM_BTAL_minus_BTALnEU_PRECT_JJA_linear_trend_1901to1955.nc"

# Variable names (None = auto-pick first 2D lat/lon variable)
GPCC_VAR = None
ALL_VAR  = None

# Box by four corners (lon,lat). Use [-180,180) or [0,360). [] = full common domain.
CORNERS = [
    (70.0, 10.0),   # SW
    (90.0, 10.0),   # SE
    (90.0, 25.0),   # NE
...
# Confidence level for two-sided CI (0.90 / 0.95 / 0.99)
CONF_LEVEL = 0.90

# Whether to also compute area-weighted Spearman (rank) correlation
DO_SPEARMAN = True

# Snap the box edges to the nearest GPCC grid points (recommended)
SNAP_TO_GRID = True

# Warn when std is below this threshold (near-constant → unstable r)
EPS_STD = 1e-8
...
# Percentile for extremes overlap (e.g., 10 = top/bottom 10%)
EXTREME_PCT = 10.0
...
def _snap_bounds_to_grid(g, lon_min, lon_max, lat_min, lat_max):
    """Snap bounds to nearest GPCC grid points."""
...
def area_weighted_spearman(x, y, conf=CONF_LEVEL):
    ...
    def rank_array(a):
        vals = a.values
        # 仅对原本是有效值的位置进行排名；无效格点保持 NaN
        mask = np.isfinite(vals)
        v = vals[mask]
        if v.size == 0:
            return xr.full_like(a, np.nan, dtype=float)

        # 先给出顺序秩
        order = np.argsort(v, kind="mergesort")
        ranks_v = np.empty_like(v, dtype=float)
        ranks_v[order] = np.arange(1, v.size + 1, dtype=float)

        # 并列值使用“平均秩”修正
        uniq, inv, cnt = np.unique(v, return_inverse=True, return_counts=True)
        cum = np.cumsum(cnt); start = cum - cnt + 1
        mean_rank = (start + cum) / 2.0
        ranks_v = mean_rank[inv]

        # 把秩填回原网格（无效位置保持 NaN）
        ranks = np.full(vals.shape, np.nan, dtype=float)
        ranks[mask] = ranks_v
        return xr.DataArray(ranks, coords=a.coords, dims=a.dims)

    Rx, Ry = rank_array(x), rank_array(y)
    r_s, _, Neff, ci = area_weighted_pearson(Rx, Ry, conf=conf)
    return r_s, Neff, ci

def weighted_fraction(mask, w):
    """Weighted fraction: sum(w * mask) / sum(w) over finite w."""
    m = np.isfinite(w) & (w > 0)
    wv = w.where(m).values.ravel()
    mv = xr.where(mask, 1.0, 0.0).where(m).values.ravel()
    good = np.isfinite(wv) & np.isfinite(mv)
    wv, mv = wv[good], mv[good]
    if wv.size == 0:
        return np.nan
    return float(np.sum(wv * mv) / np.sum(wv))

def unweighted_fraction(mask):
    """Unweighted fraction: mean(mask)."""
    mv = xr.where(mask, 1.0, 0.0).values.ravel()
    good = np.isfinite(mv)
    v = mv[good]
    if v.size == 0:
        return np.nan
...
def weighted_quantile(values, quantiles, sample_weight=None):
    """
    Weighted quantile for 1D numpy arrays.
    quantiles in [0,1]. If sample_weight is None, reduces to np.quantile.
    """
...
def main():
    ...
    # Weighted stds and ratio
    w = weights_for_da(gpcc_m)
    ...
    # --------- Additional robust agreement metrics ---------
    # Sign agreement (weighted & unweighted)
    sign_same = xr.where(xr.apply_ufunc(np.sign, gpcc_m) == xr.apply_ufunc(np.sign, all_m), True, False)
    sign_agree_unw = unweighted_fraction(sign_same)
    sign_agree_w   = weighted_fraction(sign_same, w)
    print(f"[符号一致率] 未加权 = {sign_agree_unw*100:.1f}%   面积加权 = {sign_agree_w*100:.1f}%")

    # Top/Bottom decile overlap (weighted & unweighted)
    p = EXTREME_PCT
    # unweighted thresholds
    topA_u = np.nanpercentile(gpcc_m.values, 100 - p)
    botA_u = np.nanpercentile(gpcc_m.values, p)
    topB_u = np.nanpercentile(all_m.values,  100 - p)
    botB_u = np.nanpercentile(all_m.values,  p)
    top_overlap_unw = unweighted_fraction((gpcc_m >= topA_u) & (all_m >= topB_u))
    bot_overlap_unw = unweighted_fraction((gpcc_m <= botA_u) & (all_m <= botB_u))

    # weighted thresholds
    A = gpcc_m.values.ravel()
    B = all_m.values.ravel()
    W = w.values.ravel()
    q_low, q_high = p/100.0, 1.0 - p/100.0
    botA_w, topA_w = weighted_quantile(A, [q_low, q_high], sample_weight=W)
    botB_w, topB_w = weighted_quantile(B, [q_low, q_high], sample_weight=W)
    top_overlap_w = weighted_fraction((gpcc_m >= topA_w) & (all_m >= topB_w), w)
    bot_overlap_w = weighted_fraction((gpcc_m <= botA_w) & (all_m <= botB_w), w)

    print(f"[Top {int(p)}% 重叠] 未加权 = {top_overlap_unw*100:.1f}%   面积加权 = {top_overlap_w*100:.1f}%")
    print(f"[Bottom {int(p)}% 重叠] 未加权 = {bot_overlap_unw*100:.1f}%   面积加权 = {bot_overlap_w*100:.1f}%")
