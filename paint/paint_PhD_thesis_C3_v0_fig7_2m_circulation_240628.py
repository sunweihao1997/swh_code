...
def paint_pic(extent,sst_lon,sst_lat,wind_lon,wind_lat,prect_lat,prect,sst,U2M,V2M,prect_early, prect_late, sst_early, sst_late, U2M_early, U2M_late, V2M_early, V2M_late, date,dates,number):
    ...
    for row in range(3):
        for col in range(2):
            ...
            # 等值线
            im1  =  ax.contour(sst_lon,prect_lat,prect[date[j],:],levels=np.arange(6,31,12),colors='#228B22',linewidths=3.1,alpha=1,zorder=1)
            ...
            # 流线
            #print(U2M.shape)
            q   =   ax.streamplot(wind_lon, wind_lat, U2M[date[j],:], V2M[date[j],:],linewidth=3, color = 'dimgrey',density=[1, 1.15], arrowsize=3.5, arrowstyle='->',)
    ...
    # 保存图片
    #plt.tight_layout()
    save_fig(path_out="/home/sun/paint/phd/",file_out="phd_c3_fig6_2mwind.pdf")
...
def main():
    ...
    # 选取时间
    dates  =  [-12,-6,-4,-2,0,2]  #总共6张图
    date   =  [18,24,26,28,30,32]
