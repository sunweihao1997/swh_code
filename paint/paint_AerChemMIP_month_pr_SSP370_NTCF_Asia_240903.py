...
def cal_multiple_model_avg_modified(f0, exp_tag1, exp_tag2,):
    ...
    for mm in models_label:
        ...
        single_diff = ((np.average(f0[varname1].data, axis=0)) - (np.average(f0[varname2].data, axis=0)))
...
def plot_change_wet_day(diff, left_string, figname, lon, lat, parray, ct_level=np.linspace(-10., 10., 21),):
    ...
    for row in range(1):
        ...
        # 设置刻度
        set_cartopy_tick(ax=ax,extent=extent,xticks=np.linspace(40,150,7,dtype=int),yticks=np.linspace(0,50,6,dtype=int),nx=1,ny=1,labelsize=70)
        ...
        ax.set_title(right_title[row], loc='right', fontsize=25)
...
def plot_change_wet_day_northern(diff, left_string, figname, lon, lat, parray, ct_level=np.linspace(-10., 10., 21),):
    ...
    for row in range(1):
        ...
        # 设置刻度
        set_cartopy_tick(ax=ax,extent=extent,xticks=np.arange(60,125,15,dtype=int),yticks=np.linspace(0,70,8,dtype=int),nx=1,ny=1,labelsize=25)
...
#    cb.set_ticklabels(ct_level)
    ...
    plt.savefig("/Volumes/Untitled/paint/AerChemMIP_modelgroup_spatial_MJJAS_ssp370_ntcf_{}_asia.pdf".format(figname))


if __name__ == '__main__':
    f0  =  xr.open_dataset('/Volumes/Untitled/AerChemMIP/process/multiple_model_climate_pr_month_MJJAS.nc')
...
#    ttest     =  cal_student_ttest(ssp0, ntcf0)
    ...
    plot_change_wet_day_northern(diff_avg*-86400, 'pr (MJJAS)', 'pr (MJJAS)', f0.lon.data, f0.lat.data, sign, levels)