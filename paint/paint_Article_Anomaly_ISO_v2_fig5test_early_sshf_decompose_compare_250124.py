...
This script is to compute the sum of each term within the SSHF, in order to compare with the original ERA5 sshf
relevant link: https://befitting-reason-87f.notion.site/185d5b19b11d8004b22ace7d0c9b86a4?pvs=4
'''
import xarray as xr
import numpy as np
import sys
...
# ======== File Information ==========
file0      = xr.open_dataset("/home/sun/data/climate_data/air_sea_interaction/climate_abnormal_u2m_v2m_t2m_ts.nc").sel(lat=slice(0, 10), lon=slice(55, 90))
...
#ncfile.to_netcdf("/home/sun/data/process/air_sea_interaction_quantity_0to10N.nc")

f2 = xr.open_dataset("/home/sun/data/process/air_sea_interaction_quantity_0to10N.nc").sel(lat=slice(0, 10), lon=slice(55, 90))
u0dt  = f2.u0dt.data
u0dt0 = f2.u0dt0.data
...
axs[0].plot(np.linspace(55, 90, 36), -1 * np.average(np.average(np.sum(shf_early[:, 0:10,], axis=0), axis=0), axis=0),     color='k',    marker='o', markevery=2, lw=2, markersize=7.5, alpha=1, label = "sum P1")
...
axs[1].plot(np.linspace(55, 90, 36), -1 * np.average(np.average(np.sum(shf_early[:, 10:20], axis=0), axis=0), axis=0),     color='k',    marker='o', markevery=2, lw=2, markersize=7.5, alpha=1, label = "sum P2")
...
axs[2].plot(np.linspace(55, 90, 36), -1 * np.average(np.average(np.sum(shf_early[:, 20:30], axis=0), axis=0), axis=0),     color='k',    marker='o', markevery=2, lw=2, markersize=7.5, alpha=1, label = "sum P3")
...
plt.savefig("/home/sun/paint/monsoon_onset_composite_ERA5/Article_Anomaly_ISO_v2_fig5test_sshf_decompose.pdf")