...
def cal_multiple_model_avg(f0, exp_tag, timeaxis, varname='wet_day'):
    ...
    multiple_model_value = np.zeros((models_num * 3, len(time), len(lat), len(lon))) # This array is to calculate the uncertainity

    j = 0
    for mm in models_label:
        varname1 = varname + '_' + mm + '_' + exp_tag

        multiple_model_avg += (np.average(f0[varname1].data, axis=0) / models_num)

        multiple_model_value[j]   = f0[varname1].data[0]
        multiple_model_value[j+1] = f0[varname1].data[1]
        multiple_model_value[j+2] = f0[varname1].data[2]
    ...
    multiple_model_avg_nc  =  xr.Dataset(
        {
            ...
            "multiple_model_value":   (["realization", "time", "lat", "lon"], multiple_model_value),    
        },
        coords={
            "time": (["time"], time),
            "lat":  (["lat"],  lat),
            "lon":  (["lon"],  lon),
            "realization": (["realization"], np.linspace(1, models_num * 3, models_num * 3)),
        },
        )
...
def plot_trend_uncertainity(fhist, fssp, fntcf, region_slice, type1):
    ...
    fhist_avg    = np.average(np.average(fhist_region['multiple_model_avg'].data, axis=1), axis=1)
    fssp_avg     = np.average(np.average(fssp_region['multiple_model_avg'].data, axis=1), axis=1)
    fntcf_avg    = np.average(np.average(fntcf_region['multiple_model_avg'].data, axis=1), axis=1)

    fhist_std    = np.std(np.average(np.average(fhist_region['multiple_model_value'].data, axis=2), axis=2), axis=0)
    fssp_std     = np.std(np.average(np.average(fssp_region['multiple_model_value'].data, axis=2), axis=2), axis=0)
    fntcf_std    = np.std(np.average(np.average(fntcf_region['multiple_model_value'].data, axis=2), axis=2), axis=0)
    ...
    ax.plot(np.linspace(2000, 2050, 51), avg_ssp[50:],     color='royalblue',      linewidth=3.25, alpha=1, label='SSP370')
    ax.plot(np.linspace(2000, 2050, 51), avg_ntcf[50:],    color='red',            linewidth=3.25, alpha=1, label='SSP370NTCF')

    # Paint the model deviation
    ax.fill_between(np.linspace(2000, 2050, 51), avg_ssp[50:]  + std_ssp[50:],  avg_ssp[50:]  - std_ssp[50:],  facecolor='royalblue', alpha=0.25)
    ax.fill_between(np.linspace(2000, 2050, 51), avg_ntcf[50:] + std_ntcf[50:], avg_ntcf[50:] - std_ntcf[50:], facecolor='red', alpha=0.25)
    ...
    plt.savefig(f"/home/sun/paint/AerMIP/regional_precip_index/{region_slice}_{type1}_trend_2000-2050_realization_include_for_std.png", dpi=700)
...
def main1():
    ...
    for type1 in type0:
        filename1 = 'multiple_model_climate_realization_include_' + type1 + '.nc'
