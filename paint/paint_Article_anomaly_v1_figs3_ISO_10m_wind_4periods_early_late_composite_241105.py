...
new edition 2024-11-5:
modified for the article FigureS3
note: This is only to serve for the comparison of late onset years when corresponding to the onset date in early and normal years
'''
import xarray as xr
import numpy as np
import argparse
...
#    newcmp.set_over('#145DA0')
    ...
    for row in range(4):
        ...
        im2  =  ax.contourf(lon_trmm, lat_trmm, precip[t][j], np.linspace(1, 5, 9), cmap='Blues', alpha=1,extend='max')
    ...
    for row in range(4):
        ...
        # Shading for precipitation
        im2  =  ax.contourf(lon_trmm, lat_trmm, precip[t][j], np.linspace(1, 7, 13), cmap='Blues', alpha=1, extend='max')
    ...
    plt.savefig('/home/sun/paint/monsoon_onset_composite_ERA5/paint_article_anomaly_ISO_v1_figs3_late_year_when_onset_early_normal.pdf')
# ============================================================================
...
# === Set time to paint ===
select_time = np.array([-37, -20, -7, -2])
number =  ["a","b","c","d","e"]
...
def main():
    ...
    # UV wind
    uwind_c0 = xr.open_dataset(path1 + f_uwind[0]).sel(time = slice(select_time[0], select_time[0]+3)); uwind_c1 = xr.open_dataset(path1 + f_uwind[0]).sel(time = slice(select_time[1], select_time[1]+3)) ; uwind_c2 = xr.open_dataset(path1 + f_uwind[0]).sel(time = slice(select_time[2], select_time[2]+3)) ; uwind_c3 = xr.open_dataset(path1 + f_uwind[0]).sel(time = slice(select_time[3], select_time[3]+3))
    uwind_e0 = xr.open_dataset(path1 + f_uwind[1]).sel(time = slice(select_time[0], select_time[0]+3)); uwind_e1 = xr.open_dataset(path1 + f_uwind[1]).sel(time = slice(select_time[1], select_time[1]+3)) ; uwind_e2 = xr.open_dataset(path1 + f_uwind[1]).sel(time = slice(select_time[2], select_time[2]+3)) ; uwind_e3 = xr.open_dataset(path1 + f_uwind[1]).sel(time = slice(select_time[3], select_time[3]+3))
    uwind_l0 = xr.open_dataset(path1 + f_uwind[2]).sel(time = slice(select_time[0], select_time[0]+3)); uwind_l1 = xr.open_dataset(path1 + f_uwind[2]).sel(time = slice(select_time[1], select_time[1]+3)) ; uwind_l2 = xr.open_dataset(path1 + f_uwind[2]).sel(time = slice(select_time[2], select_time[2]+3)) ; uwind_l3 = xr.open_dataset(path1 + f_uwind[2]).sel(time = slice(select_time[3], select_time[3]+3))
    vwind_c0 = xr.open_dataset(path1 + f_vwind[0]).sel(time = slice(select_time[0], select_time[0]+3)); vwind_c1 = xr.open_dataset(path1 + f_vwind[0]).sel(time = slice(select_time[1], select_time[1]+3)) ; vwind_c2 = xr.open_dataset(path1 + f_vwind[0]).sel(time = slice(select_time[2], select_time[2]+3)) ; vwind_c3 = xr.open_dataset(path1 + f_vwind[0]).sel(time = slice(select_time[3], select_time[3]+3))
    vwind_e0 = xr.open_dataset(path1 + f_vwind[1]).sel(time = slice(select_time[0], select_time[0]+3)); vwind_e1 = xr.open_dataset(path1 + f_vwind[1]).sel(time = slice(select_time[1], select_time[1]+3)) ; vwind_e2 = xr.open_dataset(path1 + f_vwind[1]).sel(time = slice(select_time[2], select_time[2]+3)) ; vwind_e3 = xr.open_dataset(path1 + f_vwind[1]).sel(time = slice(select_time[3], select_time[3]+3))
    vwind_l0 = xr.open_dataset(path1 + f_vwind[2]).sel(time = slice(select_time[0], select_time[0]+3)); vwind_l1 = xr.open_dataset(path1 + f_vwind[2]).sel(time = slice(select_time[1], select_time[1]+3)) ; vwind_l2 = xr.open_dataset(path1 + f_vwind[2]).sel(time = slice(select_time[2], select_time[2]+3)) ; vwind_l3 = xr.open_dataset(path1 + f_vwind[2]).sel(time = slice(select_time[3], select_time[3]+3))
...
#    sshf_l = xr.open_dataset(path1 + f_sshf[2]).sel(time = select_time)
    ...
    olr_f0  = xr.open_dataset("/home/sun/data/composite/early_late_composite/ERA5_precipitation_bandpass_2070_early_late_composite.nc").sel(time_composite = slice(select_timepr[0], select_timepr[0]+3))
    olr_f1  = xr.open_dataset("/home/sun/data/composite/early_late_composite/ERA5_precipitation_bandpass_2070_early_late_composite.nc").sel(time_composite = slice(select_timepr[1], select_timepr[1]+3))
    olr_f2  = xr.open_dataset("/home/sun/data/composite/early_late_composite/ERA5_precipitation_bandpass_2070_early_late_composite.nc").sel(time_composite = slice(select_timepr[2], select_timepr[2]+3))
    olr_f3  = xr.open_dataset("/home/sun/data/composite/early_late_composite/ERA5_precipitation_bandpass_2070_early_late_composite.nc").sel(time_composite = slice(select_timepr[3], select_timepr[3]+3))
