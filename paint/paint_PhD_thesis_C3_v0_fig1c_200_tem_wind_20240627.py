...
def paint_picture(lon,lat,avg_t2m,avg_u,avg_v):
    ...
    # 绘制气温等值线
    im  =  ax.contourf(lon,lat,avg_t2m-273.15,np.linspace(-40,-28,13),cmap=cmp,alpha=1,extend='both')

    # 绘制海岸线
    ax.coastlines(resolution='110m',lw=1)

    ax.set_title("300 hPa", loc='right', fontsize=22.5)
    ax.set_title("(c)", loc='left', fontsize=22.5)

    # 绘制矢量图
    q  =  ax.quiver(lon, lat, avg_u, avg_v, 
                regrid_shape=12.5, angles='uv',   # regrid_shape这个参数越小，是两门就越稀疏
                scale_units='xy', scale=1.65,        # scale是参考矢量，所以取得越大画出来的箭头就越短
                units='xy', width=0.25,
                transform=proj,
    ...
    # colorbar
    a = fig.colorbar(im,shrink=0.6, pad=0.05,orientation='vertical')
    a.ax.tick_params(labelsize=15)

    # 保存图片
    save_fig(path_out="/home/sun/paint/phd/",file_out="phd_c3_fig1c_200t_200wind.pdf")
...
def main():
    ...
    f1   =  xr.open_dataset("/home/sun/data/merra2_climate_vars_multi/monthly/merra2_climate_monthly_vars.nc").sel(lev=300)
