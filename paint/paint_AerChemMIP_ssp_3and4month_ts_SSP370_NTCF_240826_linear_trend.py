...
#    v,   clon, clat = cutil.add_cyclic(v,lon, lat)
    ...
    for row in range(1):
        ...
        im   =  ax.contourf(clon1, clat1, psl1, ct_level, cmap='Reds', alpha=1, extend='both',transform=ccrs.PlateCarree())

        #im2  =  ax.contourf(lon, lat, psl, np.linspace(-10, 0, 11), colors='grey', alpha=1,)

        # t 检验
        sp  =  ax.contourf(clon1, clat1, pva1, levels=[0., 0.01], colors='none', hatches=['.'],transform=ccrs.PlateCarree())
