    ...
    multiple_model_avg_nc  =  xr.Dataset(
        {
            ...
            "multiple_model_std":     (["time", "lat", "lon"], np.std(multiple_model_value, axis=0)),    
        },
        coords={
            "time": (["time"], time),
            "lat":  (["lat"],  lat),
...
def plot_trend_uncertainity(fhist, fssp, fntcf, region_slice, type1):
    ...
    fhist_std    = np.average(np.average(fhist_region['multiple_model_std'].data, axis=1), axis=1)
    fssp_std     = np.average(np.average(fssp_region['multiple_model_std'].data, axis=1), axis=1)
    fntcf_std    = np.average(np.average(fntcf_region['multiple_model_std'].data, axis=1), axis=1)
