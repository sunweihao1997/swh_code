...
models_label = ['EC-Earth3-AerChem', 'UKESM1-0-LL', 'GFDL-ESM4', 'MRI-ESM2','GISS-E2-1-G','MIROC6', ] # GISS provide no daily data
...
#        im  =  ax.contourf(clon2d, clat2d, cdata, ct_level, cmap=new_cmap, alpha=1, extend='both')
        ...
        im2  =  ax.contourf(lon, lat, omega, np.array([-4, -3, -2, -1, 0, 1, 2, 3, 4,]), cmap='coolwarm', alpha=1, extend='both', transform=ccrs.PlateCarree(), )
    ...
    plt.savefig("/home/sun/paint/AerChemMIP/AerChemMIP_modelgroup_spatial_MJJAS_ssp370_ntcf_{}_northern_hemisphere.png".format(figname))
...
#    print(np.nanmean(f1.Fx.data))
    ...
    plot_change_wet_day(np.nanmean(ssp0, axis=0), np.nanmean(ntcf0, axis=0), np.nanmean(omega, axis=0)*1e3, '200hPa u (MJJAS)', '200hPa u (MJJAS)', f0.lon.data, f0.lat.data, ttest, levels/10)