...
def main():
    ...
    tfile = xr.open_dataset("/home/sun/data/monsoon_onset_anomaly_analysis/ERA5_data_monsoon_onset/composite_ERA5/static_stability_composite_abnormal.nc")
    tfile_1000 = tfile.sel(lev=1000,lat=slice(10, 0), lon=slice(50, 100))
    tfile_850  = tfile.sel(lev=850, lat=slice(10, 0), lon=slice(50, 100))
    tfile_500  = tfile.sel(lev=500, lat=slice(10, 0), lon=slice(50, 100))
    tfile_600  = tfile.sel(lev=600, lat=slice(10, 0), lon=slice(50, 100))

    start0 = 20 ; end0 = 25
    sf_series1 =  np.nanmean(np.nanmean(tfile_850['theta_e_late'].data[start0:end0], axis=0), axis=1)
    sf_series2 =  np.nanmean(np.nanmean(tfile_1000['theta_e_late'].data[start0:end0], axis=0), axis=1)
    sf_series3 =  np.nanmean(np.nanmean(tfile_500['theta_e_late'].data[start0:end0], axis=0), axis=1)
    sf_series4 =  np.nanmean(np.nanmean(tfile_600['theta_e_late'].data[start0:end0], axis=0), axis=1)
...
#    print(diff.shape)
#    im  =  ax.contourf(tfile_1000.lon.data,tfile_1000.lat.data,diff*1e4,np.linspace(1, 10, 10),cmap='Reds',alpha=1,extend='both')
#
#    # 绘制温度等值线
...
#
#    # 保存图片  
#    save_fig(path_out="/home/sun/paint/phd/",file_out="phd_c5_test_diff_SB.pdf")
