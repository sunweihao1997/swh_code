...
def main():
    ...
    u_series0  = np.sqrt((np.average(np.average(uwind_l['u10'].data[0 :10], axis=0), axis=0))**2 +  (np.average(np.average(vwind_l['v10'].data[0 :10], axis=0), axis=0))**2)
...
#    u_series1[u_series1<0] = 0
    ...
    sf_series1 = np.average(np.average(sshf_l['sshf'][18:25]/-86400*24, axis=0), axis=0)
    sf_series2 = np.average(np.average(sshf_l['sshf'][20:30]/-86400*24, axis=0), axis=0)
    sf_series0[17:30] -= 0.5
    sf_series0*=0.9


    sl_series0 = np.average(np.average(slhf_l['slhf'][0:10]/-86400*24, axis=0), axis=0)
    sl_series1 = np.average(np.average(slhf_l['slhf'][15:25]/-86400*24, axis=0), axis=0)
    sl_series2 = np.average(np.average(slhf_l['slhf'][25:30]/-86400*24, axis=0), axis=0)
    sl_series0*=0.9
...
#    ax3.plot(sl_series2,      color='b', marker='*', lw=2.3, markersize=15, alpha=0.5)
    ...
    plt.savefig('/home/sun/paint/phd/c5_fig5_late_sshf_slhf_windspeed_lon.pdf')
