...
im = ax.pcolormesh(f0.lon.data, f0.lat.data, 1.05*((f0['withdraw_ssp3'].data - f1['onset_ssp3'].data) - (f0['withdraw_ntcf'].data - f1['onset_ntcf'].data)), cmap=newcmp, vmin=-20, vmax=20)
...
ax.set_title('withdraw day - onset day', loc='right', fontsize=15)

ax.coastlines(resolution='10m',lw=1.65)
plt.colorbar(im, extend='both', orientation='horizontal')

plt.savefig('/home/sun/paint/AerMIP/modelmean_rainseason_Bingwang_diff_threshold4.png')
