...
def plot_change_wind_psl(u, v, psl, left_string, figname, lon1, lat1, lon2, lat2, parray, ct_level=np.linspace(-10., 10., 21),):
    '''
...
#    from module_sun import set_cartopy_tick
    ...
    #lon = (lon + 180) % 360 - 180
    #print(lon)
    psl1, clon1, clat1 = cutil.add_cyclic(psl, lon2, lat2)
    u1,   clon, clat = cutil.add_cyclic(u,lon1, lat1)
    v1,   clon, clat = cutil.add_cyclic(v,lon1, lat1)
    #clon = lon ; clat = lat
    ...
    for row in range(1):
        ...
        # wind
        q  =  ax.quiver(clon, clat, u1, v1, 
                regrid_shape=13, angles='uv',   # regrid_shape这个参数越小，是两门就越稀疏
                scale_units='xy', scale=0.015,        # scale是参考矢量，所以取得越大画出来的箭头就越短
    ...
    plt.savefig("/home/sun/paint/AerChemMIP/AerChemMIP_modelgroup_global_MA_200divwind_ssp370_linear_trend_{}.pdf".format(figname))
...
if __name__ == '__main__':
    ...
    # -------- u --------------
    f0  =  xr.open_dataset('/home/sun/data/AerChemMIP/process/multiple_model_climate_divu200_month_MA.nc')
...
#    u         =  ssp0 
    ...
    # -------- v ---------------
    f0  =  xr.open_dataset('/home/sun/data/AerChemMIP/process/multiple_model_climate_divv200_month_MA.nc')
...
#    v         =  -1*(np.gradient(ssp0, axis=0) - np.gradient(ntcf0, axis=0))
    ...
    lon1 = f0.lon.data ; lat1 = f0.lat.data

    # -------- psl -------------
    f0  =  xr.open_dataset('/home/sun/data/AerChemMIP/process/multiple_model_climate_wap_month_MA.nc').sel(plev=50000)
...
#    ttest     =  cal_student_ttest(ssp0, ntcf0)
    ttest     =  0
    lon2 = f0.lon.data ; lat2 = f0.lat.data
    ...
    plot_change_wind_psl(u*10, v*10, psl*1e4, '200 hPa', '200 hPa', lon1, lat1, lon2, lat2, ttest, levels)