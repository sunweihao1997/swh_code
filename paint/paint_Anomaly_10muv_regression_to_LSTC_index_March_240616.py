...
def paint_picture(lon, lat, u, v, psl, corre, correu):
    ...
    q  =  ax.quiver(lon, lat, u, v, 
                ...
                scale_units='xy', scale=0.2,        # scale是参考矢量，所以取得越大画出来的箭头就越短
                units='xy', width=0.35,
                transform=proj,
    ...
    # 保存图片
    save_fig(path_out="/home/sun/paint/phd/",file_out="phd_c5_v1_fig7test_mar_regression_uv_psl_ttest.pdf")

def main():
    f1 = xr.open_dataset('/home/sun/data/monsoon_onset_anomaly_analysis/ERA5_data_monsoon_onset/regression/ERA5_regression_10uv_psl_to_LSTC_remove_OLR_ttest_March.nc')
    #print(f1)

    paint_picture(lon=f1.longitude,lat=f1.latitude,u=f1['rc_u'].data*-1, v=f1['rc_v'].data*-1, psl=f1['rc_p'].data*-1, corre=f1['p_p'].data, correu=f1['p_u'].data)
