...
f_ind = xr.open_dataset("/home/sun/data/model_data/climate/b1850_exp/b1850_maritime_atmosphere_new020.nc").sel(lev=850)
...
z_file = xr.open_dataset("/home/sun/data/geopotential/CESM_grid_interp.nc")
p_file = xr.open_dataset("/home/sun/data/model_data/climate/b1850_exp/con_minus_sen_maritime_prob.nc")
#sys.exit()
p_value = p_file.prob_apr.data
# ===========================================
...
ps    = np.average(f_ctl['PS'].data[start0:end0], axis=0)    - np.average(f_ind['PS'].data[start0:end0], axis=0)
...
M = 0.0289644  # kg/mol
R = 8.31447  # J/(mol·K)
h = z_file.z.data[0]/9.8
psl_ctl   = np.average(f_ctl['PS'].data[start0:end0], axis=0) / 100 * (1 - (L * h) / (np.average(f_ctl['TS'].data[start0:end0], axis=0) + L * h)) ** (-g * M / (R * L))
psl_ind   = np.average(f_ind['PS'].data[start0:end0], axis=0) / 100 * (1 - (L * h) / (np.average(f_ind['TS'].data[start0:end0], axis=0) + L * h)) ** (-g * M / (R * L))
...
# 绘制降水填色图
im  =  ax.contourf(f_ctl.lon.data,f_ctl.lat.data,psl,np.linspace(-10, 10, 11),cmap='coolwarm',alpha=1,extend='both')
#im2 =  ax.contourf(f_ctl.lon.data,f_ctl.lat.data, p_value, [0, 0.01], colors='none', hatches=['.'])
#im  =  ax.contourf(f_ctl.lon.data,f_ctl.lat.data,psl,np.linspace(-50, 50, 11),cmap='coolwarm',alpha=1,extend='both')
...
# 保存图片  
save_fig(path_out="/home/sun/paint/phd/",file_out="phd_c4_figs1_may_wind_psl_ctrl_minus_mari.pdf")
