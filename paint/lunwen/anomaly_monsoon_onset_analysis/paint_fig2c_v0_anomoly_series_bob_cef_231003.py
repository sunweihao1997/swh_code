...
path0 = '/home/sun/data/ERA5_data_monsoon_onset/climatic_daily_ERA5/v_component_of_wind/'
path1 = '/home/sun/data/ERA5_data_monsoon_onset/climatic_daily_ERA5_BOBSM_early_late_years/v_component_of_wind/'
...
for dddd in range(365):
    ...
    early_anomoly = np.append(early_anomoly, np.average(f1['v'].data) - np.average(f0['v'].data))
    late_anomoly  = np.append(late_anomoly,  np.average(f2['v'].data) - np.average(f0['v'].data))
...
def paint_early_late_cef_intensity(early_cef, late_cef):
    ...
    plt.savefig('/home/sun/paint/monsoon_onset_composite_ERA5/abnormal_bob_cef_series.pdf')
