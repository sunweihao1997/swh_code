...
def paint_composite_wind_sensible_precip(lat_era5, lon_era5, lat_trmm, lon_trmm, u, v, sshf, precip, extent):
    ...
    viridis = cm.get_cmap('Blues_r', 22)
...
#    newcmp.set_over('#145DA0')
    ...
    for col in range(4):
        ...
        im2  =  ax.contourf(lon_trmm, lat_trmm, precip[t][j], np.linspace(-25, 0, 11), cmap=newcmp, alpha=1, extend='both')
...
def main():
    ...
    # band-pass OLR
    olr_f  = xr.open_dataset("/home/sun/data/composite/early_late_composite/ERA5_OLR_bandpass_3080_early_late_composite.nc").sel(time=select_time)
