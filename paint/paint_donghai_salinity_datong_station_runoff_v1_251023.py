...
# 将“日期”列转为日期类型（如果尚未是）
daily_filled["日期"] = pd.to_datetime(daily_filled["日期"])
...
for name, df_site in daily_filled.groupby("站名"):
    ...
    # 左坐标轴（流量）
    ax1.bar(df_site["日期"], df_site["流量日平均"], color="lightblue", label="runoff", alpha=0.75)
    ...
    # 保存每个站的图像
    plt.savefig(f"/mnt/f/wsl_plot/donghai/salinity/{name}_日平均水位流量图.png", dpi=300)
    plt.close()
