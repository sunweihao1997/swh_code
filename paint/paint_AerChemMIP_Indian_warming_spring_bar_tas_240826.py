...
series2_indian_std = np.std(np.average(np.average(ntcf0_indian_std, axis=2), axis=2), axis=0)
...
def paint_evolution_landsea_contrast(ssp, sspntcf, ssp_std, ntcf_std, left_string, right_string,):
    ...
    ax.hist(ssp,     color='royalblue',    label='SSP370', alpha=0.5)
    ax.hist(sspntcf, color='red',          label='SSP370lowNTCF', alpha=0.5)
...
paint_evolution_landsea_contrast(series1_indian, series2_indian, series1_indian_std, series2_indian_std, ' ', ' ')
