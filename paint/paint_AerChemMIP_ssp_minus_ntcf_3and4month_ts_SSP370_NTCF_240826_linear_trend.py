...
def cal_multiple_model_avg(f0, exp_tag, timeaxis,):
    ...
    for mm in models_label:
        ...
        varname2 = mm + '_' + 'sspntcf'

        multiple_model_avg += ((f0[varname1].data - f0[varname2].data) / models_num)
    ...
    for i in range(len(lat)):
        for j in range(len(lon)):
            if np.isnan(multiple_model_avg[:, i, j]).any():
                ...
                # use scipy to conduct significant test
                #print("X_time shape:", X_time.shape)
                #print("y shape:", y.shape)
                slope, intercept, r_value, pvalue, std_err = linregress(X_time[:, 0], y)
                #print(p_value)
                p_value[i, j] = pvalue
...
#    u[(u**2+v**2)<0.0008] = np.nan
    ...
    # -------   cartopy extent  -----
    lonmin,lonmax,latmin,latmax  =  30,280,-30,30
    ...
    pva1, clon1, clat1 = cutil.add_cyclic(parray, lon, lat)
...
#    v,   clon, clat = cutil.add_cyclic(v,lon, lat)
    ...
    for row in range(1):
        ...
        # t 检验
        sp  =  ax.contourf(clon1, clat1, pva1, levels=[0., 0.05], colors='none', hatches=['.'],transform=ccrs.PlateCarree())
...
#    cb.set_ticklabels(ct_level)
    ...
    plt.savefig("/home/sun/paint/AerChemMIP/AerChemMIP_modelgroup_global_MA_ssp370_minus_ntcf_linear_trend_{}.pdf".format(figname))
...
#    v         =  ssp0
    ...
    # -------- psl -------------
    f0  =  xr.open_dataset('/home/sun/data/AerChemMIP/process/multiple_model_climate_ts_month_MA.nc')

    ssp0, pvalue      =  cal_multiple_model_avg(f0, 'ssp',  'time_ssp')
    # above function minus the value from ntcf experiment!!
    #ntcf0     =  cal_multiple_model_avg(f0, 'sspntcf', 'time_ssp')

    psl       = -1*ssp0
#    print(np.average(pvalue))
    #sys.exit()
    

    mask_file = mask_file.interp(latitude=f0.lat.data, longitude=f0.lon.data)
    psl[mask_file['lsm'].data[0]>0.05] = np.nan
    pvalue[mask_file['lsm'].data[0]>0.05] = np.nan
#    sys.exit()
#    print(psl[45, :])
...
#    ttest     =  cal_student_ttest(ssp0, ntcf0)
    ...
    levels    =  np.array([-1, -0.8, -0.6, -0.5, -0.2, 0, 0.2, 0.4, 0.6, 0.8, 1])
    levels    =  np.linspace(-0.2, 0.2, 11)
    plot_change_wind_psl(0, 0, psl*10, 'SST', 'SST', f0.lon.data, f0.lat.data, pvalue, levels)