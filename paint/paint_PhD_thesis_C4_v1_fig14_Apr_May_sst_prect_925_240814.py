...
v1 modified:
change sign of the result to positive
'''
import sys
...
# 绘制降水填色图
im  =  ax.contourf(f_ctl.lon.data,f_ctl.lat.data,1*pr*86400000,np.linspace(-5, 5, 11),cmap='coolwarm',alpha=1,extend='both')

# 绘制温度等值线
it  =  ax.contour(f_ctl.lon.data,f_ctl.lat.data,ts,np.array([0.5, 0.75, 1, 1.25, 1.5]),alpha=0.8, linewidths=2., colors='green', zorder=2, linestyles='--')
...
q  =  ax.quiver(f_ctl.lon.data,f_ctl.lat.data, 1*u, 1*v, 
            ...
            units='xy', width=0.35,
            transform=proj,
            color='k',linewidth=1.2,headlength = 5, headaxislength = 4, headwidth = 5, zorder=8)
...
# 保存图片  
save_fig(path_out="/home/sun/paint/phd/",file_out="phd_c4_v1_fig14_Apr_sst_prect_wind_ctrl_minus_inch.pdf")
