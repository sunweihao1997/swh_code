...
onset_date_file = xr.open_dataset('/home/sun/wd_14/data_beijing/monsoon_onset_anomaly_analysis/onset_day_data/1940-2022_BOBSM_onsetday_onsetpentad_level300500_uwind925.nc').sel(year=slice(1980, 2021))

index_file      = xr.open_dataset("/home/sun/wd_14/data_beijing/monsoon_onset_anomaly_analysis/process/ERA5_LSTC_1980_2021_t2m_ts_sp_psl_OLR.nc")
...
#    ax.plot(np.linspace(0, 14, 15), np.average(slow, axis=0), 'k',   label='Slow El Nino decaying')
    ...
    ax.fill_between(np.linspace(0, 14, 15), np.average(array2, axis=0) - 0.75 * np.std(array2), np.average(array2, axis=0) + 0.75 * np.std(array2), color='lightskyblue', alpha=0.35)
    ax.fill_between(np.linspace(0, 14, 15), np.average(array3, axis=0) - 0.75 * np.std(array3), np.average(array3, axis=0) + 0.75 * np.std(array3), color='lightsalmon',  alpha=0.35)
...
#    ax.set_title(right, loc='right', fontsize=17.5)
    ...
    fig.savefig('/home/sun/paint/paint_beijing/monsoon_onset_composite_ERA5/Anomaly_OLR_evolution_composite_early_late.pdf', dpi=450)
...
quick_test(-1 * (slp_climate - slp_climate)/slp_climate, -1 * (olr_early - olr_climate), -1 * (olr_late - olr_climate), -1 * (olr_fast - olr_climate), -1 * (olr_slow - olr_climate),'Maritime-Africa DIFF OLR', ' ', 'ERA5_Maritime_continent_OLR_month_evolution_normal_abnormal_year.pdf')
