...
if __name__ == '__main__':
    f0  =  xr.open_dataset('/data/AerChemMIP/process/multiple_model_climate_od550aer_month_MJJAS.nc')

    ssp0_all      =  cal_multiple_model_avg(f0, 'ssp',  'time_ssp')   
    ntcf0_all     =  cal_multiple_model_avg(f0, 'sspntcf', 'time_ssp')

    ttest_all     =  cal_student_ttest(ssp0_all, ntcf0_all)


    print(np.nanmean(ssp0_all))
    # Note that the variable in the above is three-dimension while the first is the number os the year
    levels    =  [-25, -20, -15, -10, -8, -6, -4, -3, -2, 2, 3, 4, 6, 8, 10, 15, 20, 25]
    plot_change_wet_day(np.nanmean(ssp0_all, axis=0)*1e2, np.nanmean(ntcf0_all, axis=0)*1e2, 'AOD550 (MJJAS)', 'AOD550 (MJJAS)', f0.lon.data, f0.lat.data, ttest_all, levels)
