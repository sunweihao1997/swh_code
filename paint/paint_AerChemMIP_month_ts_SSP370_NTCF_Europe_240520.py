...
def plot_change_wet_day(ssp, sspntcf, left_string, figname, lon, lat, parray, ct_level=np.linspace(-10., 10., 21),):
    ...
    # -------   cartopy extent  -----
    lonmin,lonmax,latmin,latmax  =  0,70,30,70
    ...
    for row in range(1):
        ...
        # 设置刻度
        set_cartopy_tick(ax=ax,extent=extent,xticks=np.linspace(0,70,8,dtype=int),yticks=np.linspace(30,70,9,dtype=int),nx=1,ny=1,labelsize=15)
    ...
    plt.savefig("/Volumes/Untitled/paint/AerChemMIP_modelgroup_spatial_MJJAS_ssp370_ntcf_{}_Europe.png".format(figname))
