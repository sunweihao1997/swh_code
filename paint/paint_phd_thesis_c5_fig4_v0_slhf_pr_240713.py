...
#    newcmp.set_over('#145DA0')
    ...
    for col in range(5):
        ...
        im2  =  ax.contourf(lon_era5, lat_era5, -1 * sshf[t][j] / 86400 * 24, np.linspace(100, 200, 11), cmap='Blues', alpha=1,extend='max')
    ...
    for col in range(5):
        ...
        # Shading for precipitation
        #im2  =  ax.contourf(lon_trmm, lat_trmm, precip[t][j], np.linspace(0.5, 4.5, 9), cmap=newcmp, alpha=1,)
        im3  =  ax.contourf(lon_era5, lat_era5, -1 * sshf[t][j] / 86400 * 24, np.linspace(100, 200, 11), cmap='Blues', alpha=1,extend='max')
    ...
    plt.savefig('/home/sun/paint/phd/phd_C5_fig4_v0_climate_early_late_circulation_slhf.pdf')
# ============================================================================
...
# === SSHF file name ===
f_sshf = ['slhf_composite.nc', 'slhf_composite_year_early.nc', 'slhf_composite_year_late.nc']

# === Mask file name ===
mask_file  =  xr.open_dataset('/home/sun/data/mask/ERA5_land_sea_mask_1x1.nc')

# === Set time to paint ===
select_time = np.array([-15, -10, -4, -0, 5])
number =  ["a","b","c","d","e"]
...
def main():
    ...
    select_time2 = np.array([-19, -10, -5, -0, 5])
    sshf_l = xr.open_dataset(path1 + f_sshf[2]).sel(time = select_time2)

    # band-pass OLR
    select_timepr = np.array([-15, -12, -7, -0, 5])
    olr_f  = xr.open_dataset("/home/sun/data/composite/early_late_composite/ERA5_precipitation_bandpass_3080_early_late_composite.nc").sel(time_composite=select_timepr)
    ...
    sshf_combined  = [sshf_e['slhf'].data, sshf_l['slhf'].data]
