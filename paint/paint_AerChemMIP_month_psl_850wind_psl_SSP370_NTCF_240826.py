...
models_label = ['EC-Earth3-AerChem', 'GFDL-ESM4', 'MRI-ESM2','MIROC6','UKESM1-0-LL'] # GISS provide no daily data
...
def plot_change_wind_psl(u, v, psl, left_string, figname, lon, lat, parray, ct_level=np.linspace(-10., 10., 21),):
    ...
    # -------   cartopy extent  -----
    lonmin,lonmax,latmin,latmax  =  40,170,-10,40
    ...
    plt.savefig("/home/sun/paint/AerChemMIP/AerChemMIP_modelgroup_spatial_MA_ssp370_ntcf_{}_test.pdf".format(figname))
...
if __name__ == '__main__':
    ...
    # -------- u --------------
    f0  =  xr.open_dataset('/home/sun/data/AerChemMIP/process/multiple_model_climate_ua_month_MA.nc').sel(plev=92500)
...
#    u         =  -1*(np.gradient(ssp0, axis=0) - np.gradient(ntcf0, axis=0))
    ...
    # -------- v ---------------
    f0  =  xr.open_dataset('/home/sun/data/AerChemMIP/process/multiple_model_climate_va_month_MA.nc').sel(plev=92500)
...
#    print(psl.shape)
    ...
    plot_change_wind_psl(u, v, psl, '925 hPa', '925 hPa', f0.lon.data, f0.lat.data, ttest, levels)