...
def paint_picture(lon, lat, u, v, psl, mask0, corre, correu, correv):
    # 绘制图像
    proj    =  ccrs.PlateCarree()
    fig,ax   =  plt.subplots(figsize=(13,8.5),subplot_kw=dict(projection=ccrs.PlateCarree()))

    # mask value over land
    u[mask0>0.5] = np.nan 
    v[mask0>0.5] = np.nan 
    psl[mask0>0.5] = np.nan 
    corre[mask0 > 0.5] = np.nan
    u[(correu > 0.1) & (correv > 0.1)] = np.nan
    ...
    im  =  ax.contourf(lon,lat,psl,np.linspace(-4,4,11),cmap='coolwarm',alpha=1,extend='both')

    im2 = ax.contourf(lon, lat, corre, [0, 0.1], colors='none', hatches=['.'])
    ...
    # 保存图片
    save_fig(path_out="/home/sun/paint/phd/",file_out="March_qV_to_LSTC_index_2edition.pdf")
...
#    sys.exit()
#
#    f2 = xr.open_dataset('/home/sun/data/monsoon_onset_anomaly_analysis/ERA5_data_monsoon_onset/regression/ERA5_regression_10uv_psl_to_LSTC_remove_OLR_ttest_March.nc')
#
#    f1 = f1.interp(latitude=f2.lat.data, longitude=f2.lon.data)
#    fmask = fmask.interp(latitude=f2.lat.data, longitude=f2.lon.data)
    #print(np.nanmean(f1['rc_div'].data))
    paint_picture(lon=f1.lon,lat=f1.lat,u=f1['rc_u'].data*-1e4, v=f1['rc_v'].data*-1e4, psl=f1['rc_div'].data*-1e10, mask0=fmask['lsm'].data[0], corre=f1['p_p'].data, correu=f1['p_u'].data, correv=f1['p_v'].data)
