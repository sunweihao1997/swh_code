...
f0  =  xr.open_dataset('/home/sun/data/process/analysis/AerChem/modelmean_onset_day_threshold4_8_harmonics.nc')
...
diff_onset = f0['onset_ssp3'].data - f0['onset_ntcf'].data
#print(diff_onset[:, 50])
diff_onset[diff_onset == 0.] = np.nan
im = ax.pcolormesh(f0.lon.data, f0.lat.data, -1.*(gaussian_filter(diff_onset, sigma=0.5)), cmap=newcmp, vmin=-15, vmax=15)
#df = pd.DataFrame(data=1.25*(f0['onset_ssp3'].data - f0['onset_ntcf'].data), columns=f0.lon.data, index=f0.lat.data)
#sns.heatmap(df)

#ax.set_title('multi-model mean', loc='left', fontsize=20)
...
plt.savefig('/home/sun/paint/AerMIP/Article_onset_Bingwang_diff_threshold4_ssp370_minus_ssp370ntcf.pdf')
