...
f_ind = xr.open_dataset("/home/sun/data/model_data/climate/b1850_exp/b1850_indian_climate_atmosphere3.nc").sel(lev=850)
p_file= xr.open_dataset("/home/sun/data/model_data/climate/b1850_exp/con_minus_sen_indian_prob.nc")

p_value = p_file.prob_may.data
...
pr    = np.average(f_ctl['PRECT'].data[120:150], axis=0) - np.average(f_ind['PRECT'].data[120:150], axis=0)
...
ts    = np.average(f_ctl['TS'].data[120:150], axis=0)    - np.average(f_ind['TS'].data[120:150], axis=0)
...
# 范围设置
lonmin,lonmax,latmin,latmax  =  45,130,-10,60
...
# 绘制降水填色图
im  =  ax.contourf(f_ctl.lon.data,f_ctl.lat.data,pr*86400000,np.linspace(-7,7,15),cmap='coolwarm',alpha=1,extend='both')
im2 =  ax.contourf(f_ctl.lon.data,f_ctl.lat.data, p_value, [0, 0.05], colors='none', hatches=['.'])
...
# 保存图片
save_fig(path_out="/home/sun/paint/phd/",file_out="phd_c4_fig8a_May_wind_precipitation_test.pdf")
