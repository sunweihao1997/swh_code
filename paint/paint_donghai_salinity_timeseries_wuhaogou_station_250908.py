'''
20250908
This script plots the salinity timeseries for Wuh aogou (五好沟) whose data is in a specific sheet.
...
# ========= 配置区(按你的实际情况改) =========
DATA_PATH   = Path("/mnt/f/数据_东海局/salinity/wuhaogou_chongming.xlsx")  # ← 改成你的实际文件
STATION_NAME = "五好沟"
STATION_CODE = "05518"         # 仅用于标题
DATE_COL = "日期"               # 该表头如截图
SAL_COL  = "盐度"
START = "2022-09-01 00:00:00"  # 如需全时段可改为 "1900-01-01"
END   = "2022-12-31 23:59:59"
OUTFIG = "/mnt/f/wsl_plot/donghai/salinity/05518_五好沟_20220905-20221212.png"
# =========================================

# ---------- 读取Excel（自动找到包含“五好沟”的sheet） ----------
...
# 优先精确匹配，其次模糊包含
if f"{STATION_CODE}{STATION_NAME}" in xf.sheet_names:
    target_sheet = f"{STATION_CODE}{STATION_NAME}"
elif STATION_NAME in xf.sheet_names:
    target_sheet = STATION_NAME
else:
    # 在所有sheet里查找包含“五好沟”的sheet名
    cand = [s for s in xf.sheet_names if STATION_NAME in s]
    target_sheet = cand[0] if cand else xf.sheet_names[0]  # 找不到就用第一个，避免报错
...
ax.set_title(f"Wuhaogou 2022.9.1–12.31 Salinity Time Series", fontsize=14, pad=10)
ax.set_xlabel("Time")
