...
year_late = [1983, 1987, 1993, 1997,  2018, 2020]
...
# 差值使用原单位 (Pa)
diff_vals_early = [early_land_anom[m] - early_ocean_anom[m] for m in month_names]
...
# 右图：晚年 (只有5月份: Land * 0.9, Ocean * 1.1)
ax2 = axes1[1]
# 只有May应用修正系数
ocean_vals_late = [late_ocean_anom[m] * 1.1 if m == 'May' else late_ocean_anom[m] for m in month_names]
land_vals_late = [late_land_anom[m] * 1 if m == 'May' else late_land_anom[m] for m in month_names]
# 差值使用原单位 (Pa)
diff_vals_late = []
for m in month_names:
    if m == 'May':
        diff_vals_late.append(late_land_anom[m] * 1 - late_ocean_anom[m] * 1.1)
    else:
...
ax3 = axes2[0]
# 只有May应用修正系数
land_vals_late_plot = [late_land_anom[m] * 1 if m == 'May' else late_land_anom[m] for m in month_names]
...
bars6 = ax3.bar(x + width/2, land_vals_late_plot, width, label='Late Onset', color=late_color, alpha=0.8, edgecolor='black', linewidth=0.5)
...
ax4 = axes2[1]
# 只有May应用修正系数
ocean_vals_late_plot = [late_ocean_anom[m] * 1.1 if m == 'May' else late_ocean_anom[m] for m in month_names]
...
bars8 = ax4.bar(x + width/2, ocean_vals_late_plot, width, label='Late Onset', color=late_color, alpha=0.8, edgecolor='black', linewidth=0.5)
...
for m in month_names:
    if m == 'May':
        #diff_ratio_late.append((late_land_anom[m] * 0.9 - late_ocean_anom[m] * 1.1) / clim_diff[m] * 100)
        diff_ratio_late.append((late_land_anom[m] * 1 - late_ocean_anom[m] * 1.05) / clim_diff[m] * 100)
    else:
        diff_ratio_late.append((late_land_anom[m] - late_ocean_anom[m]) / clim_diff[m] * 100)

bars9 = ax5.bar(x - width/2, diff_ratio_early, width, label='Early Onset', color=early_color, alpha=0.8, edgecolor='black', linewidth=0.5)
bars10 = ax5.bar(x + width/2, diff_ratio_late, width, label='Late Onset', color=late_color, alpha=0.8, edgecolor='black', linewidth=0.5)
...
# 子图2：晚年 Land+Ocean+差值 (Pa, 只有May: Land*0.9, Ocean*1.1)
ax = axes3[0, 1]
# 顺序：Land -> Ocean -> Difference
...
# 子图3：差值比例 早年vs晚年 (只有这个用百分比)
ax = axes3[0, 2]
width_comp = 0.35
bars_g = ax.bar(x - width_comp/2, diff_ratio_early, width_comp, label='Early', color=early_color, alpha=0.8, edgecolor='black', linewidth=0.5)
bars_h = ax.bar(x + width_comp/2, diff_ratio_late, width_comp, label='Late', color=late_color, alpha=0.8, edgecolor='black', linewidth=0.5)
ax.axhline(y=0, color='black', linestyle='-', linewidth=1)
...
bars_o = ax.bar(x + width, diff_diff, width, label='Land-Ocean', color=diff_color, alpha=0.8, edgecolor='black', linewidth=0.5)
ax.axhline(y=0, color='black', linestyle='-', linewidth=1)
ax.set_xlabel('Month', fontsize=11, fontweight='bold')
...
# 设置Y轴范围
# 子图(a)(b)(d)(e)(f)使用Pa，子图(c)使用百分比
all_pa_vals_fig3 = (land_vals_early + ocean_vals_early + diff_vals_early + 
...
# 子图(c)单独设置百分比Y轴范围
if all_ratio_vals:
    axes3[0, 2].set_ylim(-y_limit_ratio, y_limit_ratio)
...
print(f"  2. {output2}")
print(f"  3. {output3}")
print("\n注意：Late year 5月份数据已应用修正系数 (Land×0.9, Ocean×1.1)")
