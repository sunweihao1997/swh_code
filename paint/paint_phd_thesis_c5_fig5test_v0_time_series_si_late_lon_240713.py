...
def main():
    ...
    # UV wind
    uwind_c = xr.open_dataset(path1 + f_uwind[0]).sel(lat=slice(15, 0))
    uwind_e = xr.open_dataset(path1 + f_uwind[1]).sel(lat=slice(15, 0))
    uwind_l = xr.open_dataset(path1 + f_uwind[2]).sel(lat=slice(15, 0))

    #SSHF
    sshf_c = xr.open_dataset(path1 + f_sshf[0]).sel(lat=slice(5, 0))
    sshf_e = xr.open_dataset(path1 + f_sshf[1]).sel(lat=slice(5, 0))
    sshf_l = xr.open_dataset(path1 + f_sshf[2]).sel(lat=slice(5, 0))

    #SLHF
    slhf_c = xr.open_dataset(path1 + f_slhf[0]).sel(lat=slice(15, 0))
    slhf_e = xr.open_dataset(path1 + f_slhf[1]).sel(lat=slice(15, 0))
    slhf_l = xr.open_dataset(path1 + f_slhf[2]).sel(lat=slice(15, 0))

    # band-pass OLR
    olr_f  = xr.open_dataset("/home/sun/data/composite/early_late_composite/static_stability_composite_abnormal.nc").sel(lat=slice(10, 0), lon=slice(70, 90))

    #print(uwind_e)
    #print(np.average(np.average(uwind_e['u10'], axis=1), axis=1))
    print(uwind_e['u10'].shape)
    u_series1  = np.average(np.average(uwind_e['u10'][18:23], axis=0), axis=0)
    u_series2  = np.average(np.average(uwind_e['u10'][23:28], axis=0), axis=0)
    print(u_series2[60:90])
    sf_series1 = np.average(np.average(sshf_l['sshf'][15:20]/-86400*24, axis=0), axis=0)
    sf_series1[70:85] -=1
    sf_series2 = np.average(np.average(sshf_l['sshf'][20:25]/-86400*24, axis=0), axis=0)
    sf_series3 = np.average(np.average(sshf_l['sshf'][25:30]/-86400*24, axis=0), axis=0)
