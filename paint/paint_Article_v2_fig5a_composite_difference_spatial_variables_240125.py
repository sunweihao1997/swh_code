...
for i in range(721):
    for j in range(1440):
        ...
        if p_valu1 < 0.05 or p_valu2 < 0.05:
            p_array[i, j] = 0.01
        else:
            continue

# 2.3 calculate the difference
diff_sst = np.average(early_sst, axis=0) - np.average(late_sst, axis=0) ; diff_u10 = np.average(early_u10, axis=0) - np.average(late_u10, axis=0) ; diff_v10 = np.average(early_v10, axis=0) - np.average(late_v10, axis=0) ; diff_tp = np.average(early_tp, axis=0) - np.average(late_tp, axis=0)
...
ncfile.attrs['description'] = 'Created on 2025-1-27 by /home/sun/swh_code/paint/paint_Article_v2_fig5a_composite_difference_spatial_variables_240125.py. This file save the difference between the early and late composite. while the p value is the significance value of the u10/v10'


ncfile.to_netcdf("/home/sun/data/monsoon_onset_anomaly_analysis/ERA5_data_monsoon_onset/composite_ERA5/" + "composite_difference_u10_v10_tp_sst_MA.nc")

sys.exit()
# ================== 3. Painting ==========================
f0      =  xr.open_dataset("/home/sun/data/monsoon_onset_anomaly_analysis/ERA5_data_monsoon_onset/composite_ERA5/composite_difference_u10_v10_tp_sst_MA.nc")
u       =  f0["diff_u10"].data
v       =  f0["diff_v10"].data
...
u[f0["diff_pvalue"].data > 0.05] = np.nan

# 范围设置
lonmin,lonmax,latmin,latmax  =  50,120,-10,30
...
im1  =  ax.contourf(f0.longitude.data, f0.latitude.data, f0['diff_sst'].data, np.linspace(-1, 1, 11), cmap='coolwarm', alpha=1, extend='both')
...
# 绘制矢量图
q  =  ax.quiver(f0.longitude.data, f0.latitude.data, u, v, 
...
# 保存图片
save_fig(path_out="/home/sun/paint/monsoon_onset_composite_ERA5/",file_out="Article_Anomaly_ISO_v2_fig5a_surface_difference_March_and_April.pdf")
