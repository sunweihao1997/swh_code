...
def lon_avg_prect(time_slice=slice(100,201),lat_slice=slice(10,20),lon_slice=slice(65,130)):
    '''本函数对数据进行处理'''
    # model result
    control_prect   =  xr.open_dataset("/home/sun/data/model_data/climate/b1850_exp/b1850_control_climate_atmosphere.nc").sel(time=time_slice,lat = lat_slice,lon = lon_slice)
    noic_prect      =  xr.open_dataset("/home/sun/data/model_data/climate/b1850_exp/b1850_inch_climate_atmosphere.nc").sel(time=time_slice,lat = lat_slice,lon = lon_slice)
    noid_prect      =  xr.open_dataset("/home/sun/data/model_data/climate/b1850_exp/b1850_indian_climate_atmosphere3.nc").sel(time=time_slice,lat = lat_slice,lon = lon_slice)
    noicid_prect    =  xr.open_dataset("/home/sun/data/model_data/climate/b1850_exp/b1850_maritime_atmosphere_new020.nc").sel(time=time_slice,lat = lat_slice,lon = lon_slice)
    ...
    return (avg_control_prect,avg_noid_prect,avg_noic_prect,avg_noicid_prect)
...
def paint_model_hov(model,path_out,filename,time_slice=slice(100,201),lat_slice=slice(10,20),lon_slice=slice(65,130)):
    '''本函数绘制模式的四个敏感性实验结果'''
    f0      =  xr.open_dataset("/home/sun/data/model_data/climate/b1850_exp/b1850_control_climate_atmosphere.nc").sel(time=time_slice,lat = lat_slice,lon = lon_slice)
    # 设置画布
    ...
    j = 0
    name   =  ["CTRL","No_Indian","No_Indochina","No_Mari_Continent"]
    num    =  ["(a)","(b)","(c)","(d)"]
    ...
    for row in range(2):
        for col in range(2):
            ...
            ax.set_xticklabels(x_tick_labels,fontsize=27)
            ax.set_yticks(y_tick)
            ax.set_yticklabels(y_label,fontsize=27)
            if j==0:
                cf   = ax.contourf(f0.lon.values, f0.time.values,1.1*model[j], clevs, cmap=newcmp, extend='both')
                cs   = ax.contour(f0.lon.values,  f0.time.values,1.1*model[j], clevs, colors='k', linewidths=1)
            ...
            ax.set_title(name[j], loc='right', fontsize=27)
            ax.set_title(num[j], loc='left', fontsize=27)
            j += 1
