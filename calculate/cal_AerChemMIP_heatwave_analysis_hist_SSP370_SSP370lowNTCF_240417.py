...
def count_continue_hw_day_duration(array0):
    ...
    if np.sum(array0 != np.nan) < 3:
        ...
        while start_loc < len(array0) - 3:
            ...
            if array0[start_loc] == np.nan:
                start_loc += 1

                continue
            
            # If it is surpass the threshold: 
            elif array0[start_loc] != np.nan:
                # Whether it fullfil 3 days
                if np.sum(array0[start_loc:start_loc+3] != np.nan) < 3:
                    start_loc += 1
                    ...
                    while start_loc + start_loc_sub < len(array0): # Constraint the number in the length of array0
                        if array0[start_loc_sub + start_loc] != np.nan:
...
#    print(sample_lists[5])
#    testmin, testmax = align_the_ncfile(sample_lists[1])
#    print(sample_lists[5])
