...
def calculate_May_prect(f0, start_year, end_year,):
    ...
    prect_BOB = np.zeros((156,))
    prect_SCS = np.zeros((156,))
    prect_SA  = np.zeros((156,))
    ...
    for yy in range(start_year, end_year + 1):
        ...
        prect_BOB[j] = np.average(f0_BOB['pr'].data)
        prect_SCS[j] = np.average(f0_SCS['pr'].data)
        prect_SA[j]  = np.average(f0_SA['pr'].data)
...
def plot_May_prect(series, name0):
    ...
    plt.plot([1850, 2005], [np.average(series), np.average(series)], 'k-', alpha=0.25)
...
#    plt.plot([1995, 2014], [np.average(series[-20:]), np.average(series[-20:])], 'r--', alpha=0.75)
    ...
    plt.title('MPI-GE', loc='right', fontdict=font)

    plt.xlabel('Year', fontdict=font)
    plt.ylabel('Pr (mm/day)', fontdict=font)

    plt.grid(True)
    plt.xticks(np.linspace(1850, 2000, 7),)
    ...
    plt.savefig('/home/sun/paint/onset_day/MPI_LE-{}SM_May_prect_1850_2014.pdf'.format(name0))
...
def prect_series():
    input_path    = '/home/sun/data/download_data/MPI-GE/'
    ...
    for i in range(30):
        orign_file = xr.open_dataset(input_path + files[i], engine='netcdf4')

        BOB_m, SCS_m, SA_m = calculate_May_prect(orign_file, 1850, 2005)
...
def main():
    ...
    plot_May_prect(BOB * 86400, 'BOB')
    plot_May_prect(SCS * 86400, 'SCS')
    plot_May_prect(SA  * 86400, 'SEA')
