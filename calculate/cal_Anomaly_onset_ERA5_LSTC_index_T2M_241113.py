...
In this new edition, single area-averaged PSL over land and sea indexs are added
'''
import numpy as np
import xarray as xr
import os 
...
def calculate_land_sea_thermal_single(file0, land_lon, land_lat, ocean_lon, ocean_lat, varname, attribute):
    '''This function is only to calculate area-average value over land or over ocean only'''
    file_land = file0.sel(latitude=land_lat,  longitude=land_lon)
    file_ocean= file0.sel(latitude=ocean_lat, longitude=ocean_lon)
    ...
    if attribute == 'land':
        ...
        lstc      = np.nanmean(np.nanmean(file_land[varname].data, axis=1), axis=1)
    else:
        for tt in range(len(time)):
            file_land[varname].data[tt][mask_land['lsm'].data[0]<0.1]   = np.nan
            file_ocean[varname].data[tt][mask_ocean['lsm'].data[0]>0.1] = np.nan 

        lstc      = np.nanmean(np.nanmean(file_ocean[varname].data, axis=1), axis=1)
...
if __name__ == "__main__":
    ...
    lstc_42year_iob_land = np.array([]) 
    lstc_42year_iob_sea  = np.array([]) 
    ...
    for yy in np.linspace(1980, 2021, 42):
        ...
        lstc_42year_iob_land  = np.append(lstc_42year_iob_land, calculate_land_sea_thermal_single(f0, lon_range_land, lat_range_land, slice(40, 100), slice(20, -20), 'msl', 'land'))
        lstc_42year_iob_sea   = np.append(lstc_42year_iob_sea,  calculate_land_sea_thermal_single(f0, lon_range_land, lat_range_land, slice(40, 100), slice(20, -20), 'msl', 'ocean'))
    ...
    ncfile  =  xr.Dataset(
        {
            ...
            "LSTC_psl_IOB_land":   (["time"], lstc_42year_iob_land),
            "LSTC_psl_IOB_sea":    (["time"], lstc_42year_iob_sea),
    ...
    ncfile.attrs['description']  =  'This file saves the LSTC calculating from the ERA5 data, for 7090 the ocean selection is 20to-20, while the iob means the area for IOB index which is (-20-20N, 40-100E). Generated from /home/sun/swh_code/calculate/cal_Anomaly_onset_ERA5_LSTC_index_T2M_241113.py. The new edition add single part of the LSTC_IOB, indicating the influence of land and sea.'
    ncfile.to_netcdf("/home/sun/data/monsoon_onset_anomaly_analysis/process/ERA5_LSTC_1980_2021_t2m_ts_sp_psl_OLR_single_part.nc")