...
data_name = 'CMIP6_model_historical_SSP370_SSP370NTCF_monthly_PET_2015-2050.nc'
...
def plot_pet(pet_hist, pet_ssp, pet_ntcf,):
    ...
    pet        = [(pet_ssp - pet_hist), (pet_ntcf - pet_hist), (pet_ssp - pet_ntcf)]
    ct_level   = np.linspace(-0.5, 0.5, 11)
    ...
    plt.savefig("/data/paint/modelgroup_spatial_JJAS_hist_ssp370_ntcf_diff_PET.png")


def main():
    # Get the 2031-2050 period-averaged data for SSP370 and SSp370lowNTCF and 1980-2015 for historical
    pet_ssp   =  cal_JJAS(pet_file, 'pet_ssp',  np.linspace(2031, 2050, 2050 - 2031 + 1))
    pet_ntcf  =  cal_JJAS(pet_file, 'pet_ntcf', np.linspace(2031, 2050, 2050 - 2031 + 1))
    pet_hist  =  cal_JJAS_hist(pet_file, 'pet_hist', np.linspace(1980, 2014, 2014 - 1980 + 1)) 
