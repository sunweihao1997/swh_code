...
def generate_summary_statistics(all_storms_df: pd.DataFrame,
                                all_tracks_df: pd.DataFrame,
                                output_dir: str):
    """
    统计要求（按过程最大级别 max_grade_code 统计）：
    1. 不考虑热带低压（grade=1）
    2. 只统计热带风暴及以上（grade=2~6）
    3. 单独统计超强台风（grade=6）
    4. 6-11 月按“该系统在该年 6-11 月有路径点”计入当年
    5. 计算 year-by-year variation（逐年变化）
    """
    ...
    # 仅保留每个系统唯一的最大级别信息
    storm_grade_df = all_storms_df[
        ["storm_uid", "max_grade_code", "max_grade_name"]
    ].copy()

    # 提取路径时间，用于判断 6-11 月是否活跃
    tracks_stat = all_tracks_df[["storm_uid", "obs_time_utc"]].copy()
    tracks_stat["obs_year"] = tracks_stat["obs_time_utc"].dt.year
    tracks_stat["obs_month"] = tracks_stat["obs_time_utc"].dt.month

    # 6-11 月活跃过的系统（同一 storm_uid 在同一年只保留一次）
    active_6_11 = (
        tracks_stat[tracks_stat["obs_month"].between(6, 11)]
        [["storm_uid", "obs_year"]]
        .drop_duplicates()
        .merge(
            storm_grade_df,
    ...
    # 只考虑热带风暴及以上（2~6）
    active_ts_and_above_6_11 = active_6_11[
        active_6_11["max_grade_code"].isin([2, 3, 4, 5, 6])
    ].copy()

    # 每年 6-11 月统计
    yearly_6_11_stats = (
        active_ts_and_above_6_11.groupby("obs_year")
        .agg(
            # 热带风暴及以上总数
            ts_and_above_total_6_11=("storm_uid", "nunique"),

            # 超强台风总数
            super_typhoon_total_6_11=("max_grade_code", lambda s: s.eq(6).sum()),
        )
        .reset_index()
        .rename(columns={"obs_year": "year"})
        .sort_values("year")
        .reset_index(drop=True)
    )

    # year-by-year variation
    yearly_6_11_stats["ts_and_above_yoy_change"] = (
        yearly_6_11_stats["ts_and_above_total_6_11"].diff()
    )
    yearly_6_11_stats["ts_and_above_yoy_pct"] = (
        yearly_6_11_stats["ts_and_above_total_6_11"].pct_change() * 100
    )

    yearly_6_11_stats["super_typhoon_yoy_change"] = (
        yearly_6_11_stats["super_typhoon_total_6_11"].diff()
    )
    yearly_6_11_stats["super_typhoon_yoy_pct"] = (
        yearly_6_11_stats["super_typhoon_total_6_11"].pct_change() * 100
    )

    yearly_6_11_stats["ts_and_above_yoy_pct"] = yearly_6_11_stats["ts_and_above_yoy_pct"].round(2)
    yearly_6_11_stats["super_typhoon_yoy_pct"] = yearly_6_11_stats["super_typhoon_yoy_pct"].round(2)

    yearly_6_11_stats.to_csv(
        summary_dir / "yearly_6_11_summary.csv",
        index=False,
        encoding="utf-8-sig"
    )
    ...
    # 全时段不同级别总数（只含 2~6）
    grade_total_stats = (
        all_storms_df[all_storms_df["max_grade_code"].isin([2, 3, 4, 5, 6])]
        .groupby(["max_grade_code", "max_grade_name"])
        .size()
        .reset_index(name="storm_count")
        .sort_values("max_grade_code")
    )

    # 保证级别顺序稳定
    grade_total_stats["max_grade_name"] = grade_total_stats["max_grade_code"].map(grade_map)

    grade_total_stats.to_csv(
        summary_dir / "grade_total_summary.csv",
        index=False,
        encoding="utf-8-sig"
    )

    # 每年 6-11 月不同级别总数（只含 2~6）
    grade_yearly_6_11_stats = (
        active_ts_and_above_6_11
        .groupby(["obs_year", "max_grade_code", "max_grade_name"])
        .size()
        .reset_index(name="storm_count")
        .rename(columns={"obs_year": "year"})
        .sort_values(["year", "max_grade_code"])
    )

    grade_yearly_6_11_stats["max_grade_name"] = grade_yearly_6_11_stats["max_grade_code"].map(grade_map)

    grade_yearly_6_11_stats.to_csv(
        summary_dir / "yearly_6_11_grade_summary.csv",
        index=False,
        encoding="utf-8-sig"
    )

    print("\n========== 统计摘要 ==========")
    print("每年 6-11 月热带风暴及以上 / 超强台风 总数及逐年变化：")
    print(yearly_6_11_stats.to_string(index=False))

    print("\n不同级别热带气旋总数（仅统计热带风暴及以上，按过程最大级别统计）：")
    print(grade_total_stats.to_string(index=False))

    print(f"\n统计结果已输出到：{summary_dir}")
...
def batch_process_cma_best_track(input_dir: str, output_dir: str):
    ...
    for txt_file in txt_files:
        ...
        if issues_df.empty:
            ...
            print(
                f"  {year}: 发现并清洗 {issue_count} 条异常记录，"
                f"涉及 {issue_storms} 个台风，已写入 {log_outfile.name}"
            )
    ...
    print(f"总系统数：{len(all_storms_df)}")
    print(f"总路径记录数：{len(all_tracks_df)}")
    print(f"异常清洗记录数：{len(all_issues_df)}")
    print(f"输出目录：{output_dir}")

    # 统计输出
    generate_summary_statistics(all_storms_df, all_tracks_df, output_dir)
