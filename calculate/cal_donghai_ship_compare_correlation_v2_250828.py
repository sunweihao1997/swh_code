...
v2: Considering the bad correlation of wind speed, I doubt it was mainly due to the small value of wind speed (most values < 5 m/s). So I only consider the data with wind speed > 5 m/s.
'''
import pandas as pd
import glob
import numpy as np
...
for file in files:
    ...
    values_wind_speed = df["风速"].tolist()
    values_wind_direction = df["风向"].tolist()
...
# 调整子图之间的间距
plt.tight_layout()
plt.savefig("/mnt/f/wsl_plot/correlation_scatter_screen_windspeed.png", dpi=500)