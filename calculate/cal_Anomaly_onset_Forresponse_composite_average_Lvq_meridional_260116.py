...
6. Each panel shows: composite curve and climatology

Output: 2×3 panel figure
- Row 1: Early onset years (Feb, Mar, Apr)
- Row 2: Late onset years (Feb, Mar, Apr)
- Each panel: composite (solid) + climatology (dashed)
...
def plot_meridional_profiles_with_anomaly(profiles: Dict[str, Dict[int, Dict[str, xr.DataArray]]],
    ...
    """
    Create 2x3 panel plot showing composite, climatology, and anomaly.
    ...
    - Each panel: composite (solid) + climatology (dashed)
    """
    fig, axes = plt.subplots(2, 3, figsize=figsize, constrained_layout=True, sharey='row')
    ...
    # Row labels
    for i, onset_type in enumerate(onset_types):
        years = early_years if onset_type == 'early' else late_years
        label = f'{onset_type.capitalize()} Onset\n({len(years)} years)'
...
#        f'(Red shading: positive anomaly, Blue shading: negative anomaly)',
