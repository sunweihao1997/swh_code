...
# ============ Read index file =======================
f1 = xr.open_dataset('/home/sun/data/ERA5_data_monsoon_onset/regression/regression_uv_to_OLR_to_LSTC_remove_each_other_FMA_avg.nc')
print(f1)
...
for t in range(6):
    for i in range(181):
        for j in range(360):
            ...
            correlation_u_LSTC[t, i, j] = stats.pearsonr(f1['lstc_remove_olr'].data - np.average(f1['lstc_remove_olr']), f1['u_detrend'][:, t, i, j] - np.average(f1['u_detrend'][:, t, i, j]))[0]
            correlation_v_LSTC[t, i, j] = stats.pearsonr(f1['lstc_remove_olr'].data - np.average(f1['lstc_remove_olr']), f1['v_detrend'][:, t, i, j] - np.average(f1['v_detrend'][:, t, i, j]))[0]
            correlation_u_OLR[t, i, j]  = stats.pearsonr(f1['olr_remove_lstc'].data - np.average(f1['olr_remove_lstc']), f1['u_detrend'][:, t, i, j] - np.average(f1['u_detrend'][:, t, i, j]))[0]
            correlation_v_OLR[t, i, j]  = stats.pearsonr(f1['olr_remove_lstc'].data - np.average(f1['olr_remove_lstc']), f1['v_detrend'][:, t, i, j] - np.average(f1['v_detrend'][:, t, i, j]))[0]
...
ncfile.to_netcdf("/home/sun/data/ERA5_data_monsoon_onset/correlation/correlation_FMA_LSTC_OLR_925wind_pentad_remove_each_other.nc")
