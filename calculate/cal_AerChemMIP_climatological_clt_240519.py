...
path_src = '/data/AerChemMIP/process/post-process/clt_samegrid/'
...
## --------------------------------------------------------------------------------------------------------------------        
            if len(group_ssp) == 3:
                ssp_average  = (group_ssp[0]['clt'].data + group_ssp[1]['clt'].data + group_ssp[2]['clt'].data) / 3
                ntcf_average = (group_ntcf[0]['clt'].data + group_ntcf[1]['clt'].data + group_ntcf[2]['clt'].data) / 3
            elif len(group_ssp) == 1:
                ssp_average  = group_ssp[0]['clt'].data
                ntcf_average = group_ntcf[0]['clt'].data
            else:
...
#
            ...
            print('=============================================================')
#        
        dataset_allmodel.attrs['description'] = 'Created on 2024-6-3. This file includes the counts of the clt for single model, covering historical, SSP370 and SSP270lowNTCF experiments. All the variables is climatological, which is 1980-2014 for hist and 2031-2050 for SSP370.'
        dataset_allmodel.to_netcdf('/data/AerChemMIP/process/multiple_model_climate_clt_month_MJJAS.nc')
