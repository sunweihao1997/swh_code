'''
2024-1-31
This script is to calculate the monsoon onset dates using zonal wind criterion for the BOB and SCS area
...
src_path = '/home/sun/ERA5_data/uwind/ERA5_uwind_850_200/'
...
def BOB_monsoon_onset_time_series():
    ...
    BOB_u850_total = np.zeros((83, 365))
    ...
    for yyyy in range(1940, 2023):
        ...
        for ffff in year_list:
            ...
            BOB_u850 = np.append(BOB_u850, BOB_monsoon_onset_criterion_u850(f0, extent))

        BOB_u850_total[j, :] = BOB_u850[:365]
...
def BOB_monsoon_onset_criterion_u850(f0, extent):
    '''
        This function is to calculate the BOBSM onset dates using U850 criterion
    '''
    ...
    f0_BOB = f0.sel(latitude=slice(extent[1], extent[0]), longitude=slice(extent[2], extent[3]), level=850)

    return np.nanmean(f0_BOB['u'].data)
...
def main():
    ...
    #time_series_u850 = BOB_monsoon_onset_time_series()
