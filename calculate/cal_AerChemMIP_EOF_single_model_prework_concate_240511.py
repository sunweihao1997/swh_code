...
def EOF_analysis(input_data, lat):
    ...
    eof     = solver1.eofsAsCorrelation(neofs=2)
    pc      = solver1.pcs(npcs=2, pcscaling=1)
    var     = solver1.varianceFraction(neigs=2)

    return eof, pc, var

if __name__ == '__main__':
    ntcf_concatenate_high, ntcf_concatenate_low = concatenate_arrays(ntcf_list[0:1], 'furt')
    hist_concatenate_high, hist_concatenate_low = concatenate_arrays(hist_list[0:1], 'historical')
    ssp3_concatenate_high, ssp3_concatenate_low = concatenate_arrays(ssp3_list[0:1], 'furt')

    print(ntcf_concatenate_high.shape)
    ...
    ncfile['ntcf_high_eof'] = xr.DataArray(data=eof, dims=["num", "lat", "lon"],
                                    coords=dict(num=(["num"], [1, 2]), lon=(["lon"], lon), lat=(["lat"], lat)))
    ncfile['ntcf_high_pc']  = xr.DataArray(data=pc, dims=["time0", "num"],
                                    coords=dict(num=(["num"], [1, 2])))
    ...
    ncfile['hist_high_eof'] = xr.DataArray(data=eof, dims=["num", "lat", "lon"],coords=dict(num=(["num"], [1, 2]), lon=(["lon"], lon), lat=(["lat"], lat)))
    ncfile['hist_high_pc']  = xr.DataArray(data=pc, dims=["time1", "num"],coords=dict(num=(["num"], [1, 2])))
    ncfile['hist_high_var'] = xr.DataArray(data=var, dims=["num"], coords=dict(num=(["num"], [1, 2])))

    eof, pc, var = EOF_analysis(hist_concatenate_low, lat)
    ncfile['hist_low_eof'] = xr.DataArray(data=eof, dims=["num", "lat", "lon"],coords=dict(num=(["num"], [1, 2]), lon=(["lon"], lon), lat=(["lat"], lat)))
    ncfile['hist_low_pc']  = xr.DataArray(data=pc, dims=["time1", "num"],coords=dict(num=(["num"], [1, 2])))
    ncfile['hist_low_var'] = xr.DataArray(data=var, dims=["num"],coords=dict(num=(["num"], [1, 2])))

    # -------------------- SSP370lowNTCF ------------------------
    eof, pc, var = EOF_analysis(ssp3_concatenate_high, lat)
    ncfile['ssp3_high_eof'] = xr.DataArray(data=eof, dims=["num", "lat", "lon"],coords=dict(num=(["num"], [1, 2]), lon=(["lon"], lon), lat=(["lat"], lat)))
    ncfile['ssp3_high_pc']  = xr.DataArray(data=pc, dims=["time2", "num"],coords=dict(num=(["num"], [1, 2])))
    ncfile['ssp3_high_var'] = xr.DataArray(data=var, dims=["num"], coords=dict(num=(["num"], [1, 2])))

    eof, pc, var = EOF_analysis(ssp3_concatenate_low, lat)
    ncfile['ssp3_low_eof'] = xr.DataArray(data=eof, dims=["num", "lat", "lon"],coords=dict(num=(["num"], [1, 2]), lon=(["lon"], lon), lat=(["lat"], lat)))
    ncfile['ssp3_low_pc']  = xr.DataArray(data=pc, dims=["time2", "num"],coords=dict(num=(["num"], [1, 2])))
    ncfile['ssp3_low_var'] = xr.DataArray(data=var, dims=["num"],coords=dict(num=(["num"], [1, 2])))

    ncfile.to_netcdf('/home/sun/data/process/analysis/AerChem/EoFs_single_realization.nc')
