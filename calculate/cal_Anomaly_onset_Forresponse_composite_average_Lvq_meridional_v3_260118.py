...
def plot_meridional_profiles_with_anomaly(profiles: Dict[str, Dict[int, Dict[str, xr.DataArray]]],
    ...
    for i, onset_type in enumerate(onset_types):
        for j, month in enumerate(months):
            ...
            # Add panel label (a-i) to top left
            ax.text(0.02, 0.98, panel_labels[i][j], transform=ax.transAxes,
    ...
    for j, month in enumerate(months):
        ...
        # Add panel label (g-i) to top left
        ax.text(0.02, 0.98, panel_labels[2][j], transform=ax.transAxes,
               fontsize=12, fontweight='bold', va='top', ha='left',
               bbox=dict(boxstyle='round,pad=0.3', facecolor='white', edgecolor='none', alpha=0.8))
