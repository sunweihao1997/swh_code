...
if __name__ == '__main__':
    ...
    for i in range(len(ref_file.latitude.data)):
        for j in range(len(ref_file.longitude.data)):
            olr[:, i, j] = band_pass_calculation(olr[:, i, j], 1, 70, 20, 5)
...
#    BOB_olr   = np.average(np.average(olr, axis=1), axis=1)
    ...
    ncfile.to_netcdf("/home/sun/data/monsoon_onset_anomaly_analysis/process/ERA5_BOB_OLR_bandpass_filter_20_70_remove_seasonal.nc")